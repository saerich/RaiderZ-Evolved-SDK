...
#ifndef KY_FINDNODESFROMPOSITIONS_DEFAULT_H
#define KY_FINDNODESFROMPOSITIONS_DEFAULT_H
...
/*!	This implementation of IFindNodesFromPositions selects as the starting vertex for the next
	...
	the list is chosen.
	\pp This class is essentially equivalent to FindNodesFromPositions_NeareastReacheable, however it tends to offer
	better performance. It begins by sorting the Graph vertices according to the current constraint set for the
	PathFinder, instead of using the more expensive LpfConstraint. The Local PathFinding cost is therefore computed for
	the fewest possible candidate vertices.
	\ingroup kyruntime_pathfinding */
class FindNodesFromPositions_Default: public IFindNodesFromPositions
{
public:
	enum Status
	{
...
public:
	KY_DECLARE_FINDNODESFROMPOSITIONS(FindNodesFromPositions_Default)

public:
	/*! \label_constructor
		\param searchSpaceSize  Determines the amount of memory allocated by FindNodesFromPositions_Default
								to store candidate vertices retrieved from the GraphManager.
								\units           bytes
								\acceptedvalues  any positive value
								\defaultvalue    \c 128 */
	explicit FindNodesFromPositions_Default(KyUInt32 searchSpaceSize = 128);

	/*! \label_virtualdestructor */
	virtual ~FindNodesFromPositions_Default();
	...
	/*! FindStartNode() passes this method to GraphManager::FindNearbySortedVertexSafePtrs() in order to sort
		...
		transparently invokes the IConstraint currently in use by the PathFinder.
		You can redefine this method in a derived class in order to prioritize a different set of criteria.
	...
	KyBool GetCostFunc_Start(const Vec3f& pos, const GraphVertexPtr& vec, KyFloat32& cost);
	...
	/*! FindDestNode() passes this method to GraphManager::FindNearbySortedVertexSafePtrs() in order to sort
		...
		\param[out] cost	Stores the cost computed by this method for the specified vertex.
		\return #KY_TRUE if the \c cost parameter was successfully updated, or #KY_FALSE otherwise. */
	KyBool GetCostFunc_Dest(const Vec3f& pos, const GraphVertexPtr& vec, KyFloat32& cost);
	...
	KyBool CanGo_Start(const PointWrapper& pos, const GraphVertexPtr& node);
	...
	KyBool CanGo_Dest(const GraphVertexPtr& node, const PointWrapper& pos);


	void InitStart(const GraphManager& graphManager, const PointWrapper& start, GraphVertexSafePtr& startNodeSafePtr);
	void InitDest(const GraphManager& graphManager, const PointWrapper& dest, GraphVertexSafePtr& startNodeSafePtr);

	KyBool GetLPFCost_Start(const Vec3f& pos, const GraphVertexPtr& vec, KyFloat32& cost);
	KyBool GetLPFCost_Dest(const Vec3f& pos, const GraphVertexPtr& vec, KyFloat32& cost);
	KyBool CanGo_LPF(const PointWrapper& pw_start, const GraphVertexPtr& node);
protected:
	...
	KyFloat32* m_searchSpaceCost; //< array of costs for the candidate
	GraphVertexSafePtr m_bestLpfVertexSafePtr; //< best vertexPtr with lpf involved
	KyFloat32 m_bestCostLpf; //< best cost with lpf involved
...
#endif // KY_FINDNODESFROMPOSITIONS_DEFAULT_H
