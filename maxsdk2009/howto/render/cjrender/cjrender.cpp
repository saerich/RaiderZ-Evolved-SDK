...
int CJRenderer::Open(INode *scene,INode *vnode,ViewParams* viewPar,RendParams& rpar,HWND hwnd,DefaultLight* defaultLights,int numDefLights, RendProgressCallback* prog)
{
	int idx;
	...
	getmaps.BeginEnumeration(false); // use list to track visited refs

	if (rendParams.atmos)
		rendParams.atmos->EnumRefHierarchy(getmaps);

	if (rendParams.envMap)
		rendParams.envMap->EnumRefHierarchy(getmaps);

	getmaps.EndEnumeration();
...
void CJRenderer::Close(HWND hwnd, RendProgressCallback* prog)
{
	int idx;
...
void CJRenderer::BeginThings()
{
	...
	// Call RenderBegin() on each object.
	for (idx = 0; idx < instTab.Count(); idx++) {
		ReferenceMaker* rm = instTab[idx]->GetINode();
		if (rm)
			rm->EnumRefHierarchy(beginEnum);
	}

	// reference hierarchy from Atmospherics
	if (rendParams.atmos)
		rendParams.atmos->EnumRefHierarchy(beginEnum);

	// reference hierarchy from Environment map
	if (rendParams.envMap)
		rendParams.envMap->EnumRefHierarchy(beginEnum);

	beginEnum.EndEnumeration();
}
...
void CJRenderer::EndThings()
{
	...
	for (idx = 0; idx < instTab.Count(); idx++) {
		ReferenceMaker* rm = instTab[idx]->GetINode();
		if (rm)
			rm->EnumRefHierarchy(endEnum);
	}

	if (rendParams.atmos)
		rendParams.atmos->EnumRefHierarchy(endEnum);

	if (rendParams.envMap)
		rendParams.envMap->EnumRefHierarchy(endEnum);

	endEnum.EndEnumeration();
}
...
RefResult CJRenderer::NotifyRefChanged(Interval changeInt, RefTargetHandle hTarget, PartID& partID,  RefMessage message) {
