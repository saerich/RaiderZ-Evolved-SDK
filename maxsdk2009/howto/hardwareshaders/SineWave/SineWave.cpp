...
static TSTR FindMapFile(TCHAR *file)
{
	...
	{
		found = SearchPath(GetCOREInterface()->GetMapDir(i), file, NULL, MAX_PATH, filepath, &fpart);
		if (found != 0)
				break;
	}

   return TSTR(found ? filepath : _T(""));
}
...
HRESULT IDX9SineWave::Initialize(Mesh *mesh, INode *node)
{
	...
	if (!lmd->GetValid()) {
		...
		DebugPrint("DX9 Vertex Shader Init\n");

		// VertexShader Declarations
		TSTR vertexShaderPath = FindMapFile("sinewave.njv");
      ...
      TSTR cubeMapPath = FindMapFile("sky_cube_mipmap.dds");
