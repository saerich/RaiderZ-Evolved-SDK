...
void* SimpleParticle::GetInterface(ULONG id)
	{
	if (id==I_SIMPLEPARTICLEOBJ) {
		return (SimpleParticle*)this;
	} else 	if (id==I_PARTICLEOBJ) {
		return (ParticleObject*)this;
	} else {
...
static INode *FindObjectsNode(ReferenceTarget *ths, Object *obj) 
	{
	RefList &rlist = ths->GetRefList();
	RefListItem *ptr = rlist.first;
	while (ptr) {
		if (ptr->maker) {
			if (ptr->maker->SuperClassID()==BASENODE_CLASS_ID) {
				// Verify that we are this node's object
				INode  *node   = (INode*)ptr->maker;
				Object *nobj   = node->GetObjectRef();
				if (nobj) nobj = nobj->FindBaseObject();
				if (nobj==obj)
					return (INode*)ptr->maker;
				}

			if (ptr->maker->IsRefTarget()) {
				INode *node = FindObjectsNode((ReferenceTarget*)ptr->maker, obj);
				if (node) return node;
				}
			}
		ptr = ptr->next;
