...
void PainterInterface::LoadWinTabDLL()
	...
	if (hWinTabDLL)
		...
		PaintWTClose = (WTCLOSE)GetProcAddress(hWinTabDLL,"WTClose");;
   		}
	}
	...
	if (hitState)
		...
		if (inPaintMode)
			...
			if (inStrChange)
				...
				TSTR strn;
				strn.printf("%s %0.2f", GetString(IDS_GIZMO_STRENGTH), str);
				ipoint.x += 30;
				ipoint.y -= 30;
			...
			if (inRadiusChange)
				...
				gw->wTransPoint(&worldSpaceHit, &ipoint);
				TSTR strn;
				strn.printf("%s %0.2f", GetString(IDS_GIZMO_RADIUS), radius);
				ipoint.x += 30;
				ipoint.y -= 30;
				gw->wText(&ipoint, strn);
				}
...
void* PainterInterface::GetInterface(ULONG id)
{
	switch(id)
	{
		case PAINTERINTERFACE_V5 : return (IPainterInterface_V5*) this;
			break;
		case PAINTERINTERFACE_V7 : return (IPainterInterface_V7*) this;
			break;
...
BOOL  PainterInterface::InitializeCallback(ReferenceTarget *canvas)
	...
	if (canvas != NULL)
		this->canvas_7 = (IPainterCanvasInterface_V7 *) canvas->GetInterface(PAINTERCANVASINTERFACE_V7);
	else this->canvas_7 = NULL;
...
//		DestroyTabletWindow();  took this out I am now hangin onto this window instead of closing opening it
	//	canvas = NULL;
	//end paint mode
