...
{
	// pasteBitmap <src_bitmap> <dest_bitmap> (<box2> | <point2>) <point2> maskColor:<color> type:{#paste | #composite | #blend} alphaMultiplier:<float>
	...
	*/
	...
	{
		for (int iy=0; iy < numY; iy++,fromY++,destY++) 
		{
			...
			if (type == n_blend || type == n_composite)
			{
				...
				for (ix=fromX, xcount=0; xcount < numX; ix++,xcount++)
				{
	...
	else
	{
		...
		for (int iy=0; iy < numY; iy++,fromY++,destY++) 
		{
			...
			if (type == n_blend || type == n_composite)
				targ_bm->bm->GetLinearPixels(destX,destY,numX,work2.Ptr());
			for (ix=destX, xcount=0; xcount < numX; ix++,xcount++)
			{	
				BMM_Color_64* p = work.Ptr() + xcount;
				if ((p->r & 0xff00) != maskColor.r || (p->b & 0xff00) != maskColor.b || (p->g & 0xff00) != maskColor.g)
				{
					...
					targ_bm->bm->PutPixels(ix,destY,1,p);
				}
			}
		}
	}
...
getBitmapInfo_cf(Value** arg_list, int count)
{
	...
	else
	{
		...
		TSTR fn = file_name->to_filename();
		TCHAR  filepath[MAX_PATH];
		TCHAR* fpart;

		// search in the package, IMAGE and then the MAPS directories
		BOOL found = 
			MSZipPackage::search_current_package(fn, filepath) ||
			SearchPath(MAXScript_interface->GetDir(APP_IMAGE_DIR), fn, NULL, MAX_PATH, filepath, &fpart);
		if (!found) 
		{
			for (int i = 0; i < TheManager->GetMapDirCount(); i++) 
			{
				found = SearchPath(TheManager->GetMapDir(i), fn, NULL, MAX_PATH, filepath, &fpart);
				if (found)
					break;
			}
		}
		...
		if (bm == NULL)
			throw RuntimeError (_T("unable to find bitmap file: "), fn);
		deleteBitmap = true;
	}
...
pathIsNetworkPath_cf(Value** arg_list, int count)
{
	...
	return bool_result(PathIsNetworkPath(arg_list[0]->to_filename()));
}
