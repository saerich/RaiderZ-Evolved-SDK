             ...
             case IDC_LOGLUV:
               ...
               //Type takes priority over compression, so set it without checking compression type.
                ...
                if (IsDlgButtonChecked(hWnd, IDC_SAVE_ALPHA)
                   && (type == tif_write_color || type == tif_write_16bit))
   ...
   if(s != NULL) {
      ...
      if(s->Allocate(fbi, manager, BMM_OPEN_R)) {
         ...
         if(td->td_photometric == PHOTOMETRIC_LOGL) {
            for(int y = 0; y < height; y++) {
               if(TIFFReadScanline(tif, loadbuf, y, 0) >= 0) {
                  if(!s->Put16Gray(0, y, width, (float*)loadbuf)) {
                     success = false;
                     break;
                  }
               }
            ...
            for(int y = 0; y < height; y++) {
               if(TIFFReadScanline(tif, loadbuf, y, 0) >= 0) {
                  ...
                  if(!s->PutPixels(0, y, width, &(bmmPixelBuffer[0]))) {
                     success = false;
                     break;
                  }
               }
   ...
   do {
      ...
      for (j=0; j<rps; j++) {
         ...
         if (UserData.writeType == tif_write_color) {          /* color tiff */
            ...
            int type = 0;
            if (Storage()->IsHighDynamicRange() && Storage() && Storage()->GetStoragePtr(&type) && type == BMM_LOGLUV_32)
            {
...
BitmapIO_TIF::SaveTIF() {
   ...
   scanline = reinterpret_cast<BMM_Color_64*>(sline.Ptr()); // Cast is safe because buffer is garanteed to be large enough
...
void MAXErrorHandler(const char* module, const char* fmt, va_list ap)
{
   ...
   _sntprintf(title, bufferSize, titleFormat.data(), (module == NULL) ? defaultModule.data() : module);
   _vsntprintf(message, bufferSize, (fmt != NULL) ? fmt : _T(""), ap);
   ...
   if (!TheManager->SilentMode()) {
      MessageBox(GetFocus(), message, title, MB_OK | MB_ICONINFORMATION);
   }
   else {
      TheManager->Max()->Log()->LogEntry(SYSLOG_ERROR,NO_DIALOG,NULL,_T("%s - %s"),title,message);
   }
