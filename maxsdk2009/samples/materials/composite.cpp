...
void Composite::Dialog::RenameLayer( LayerDialog* layerDialog ) {
		...
		static INT_PTR CALLBACK proc( HWND h, UINT msg, WPARAM w, LPARAM l ) {
			...
			case WM_INITDIALOG:
				...
				SendMessage(GetDlgItem( Dialog->m_hWnd, IDC_LAYERNAME ),
					EM_SETSEL,
					0, MAKELONG(0, strlen(Dialog->m_Layer->Name())) );
				return FALSE;
				break;
	...
	int  index( m_LayerRollupWnd->GetPanelIndex( wnd ) );
	m_LayerRollupWnd->SetPanelTitle( index, const_cast<LPSTR>(layerDialog->Title().c_str()) );
}
...
void Composite::Dialog::ReloadDialog() {
	...
	for( iterator it  = begin(); it != end(); ++it ) {
		...
		m_LayerRollupWnd->SetPanelTitle( Index, const_cast<LPSTR>(it->Title().c_str()) );
...
Point3 Composite::Texture::EvalNormalPerturb(ShadeContext& sc) {
	...
	// We cheat a little here. We use Point4 with the fourth parameter
	// as an alpha.
	...
	public:
		...
		Point4 operator()( const Point4& bg, const value_type& Layer ) {
			// If the foreground is invisible, return the background.
			if (false == Layer.IsVisible())
				return bg;

			// Take normal, apply Opacity, check Mask, Blend, rinse and re-do
			Point4 fg( Layer.Texture()->EvalNormalPerturb( sc ),
				Layer.Texture()->EvalColor( sc ).a        );

			if (Layer.IsMaskVisible())
				fg.w *= Layer.Mask()->EvalMono( sc );

			fg.w *= Layer.NormalizedOpacity();
			...
			const float alpha( fg.w + bg.w * (1 - fg.w) );

			// We composite. This is NOT a real Composition as with the EvalColor
			// method. This is copied from the previous composite texmap as is
			// in order to give the same result. Basically, the normal composition
			// of colors cannot be applied to vectors so we're using the old code
			// temporarily.
			// Please note that we do NOT support blending either.
			const float retX( fg.x + bg.x * (1 - fg.w) );
			const float retY( fg.y + bg.y * (1 - fg.w) );
			const float retZ( fg.z + bg.z * (1 - fg.w) );

			return Point4( retX, retY, retZ, alpha );
		}
	};

	result = Point3(std::accumulate( begin(), end(), Point4(0, 0, 0, 0), accumulator(sc) ));
...
void Composite::Texture::InitMultimap( TimeValue  time,
	...
	for( int i = 0; i < nb_tex && it != end(); ++it ) {
		if (it->IsVisible()) {
			BitmapInfo bi;
			bi.SetWidth     ( callback.Size() );
			bi.SetHeight    ( callback.Size() );
			bi.SetType      ( BMM_TRUE_32   );
			bi.SetFlags     ( MAP_HAS_ALPHA );
			bi.SetCustomFlag( 0 );

			SmartInterface<Bitmap> tex_bm ( TheManager->Create( &bi ) );
			GetCOREInterface()->RenderTexmap( it->Texture(), tex_bm );

			PBITMAPINFO    bmi ( callback.BitmapToDIB( tex_bm, 0, 0 ) );
			unsigned char *data( (unsigned char*)bmi + sizeof( BITMAPINFOHEADER ) );

			const int      w   ( tex_bm->Width () );
			const int      h   ( tex_bm->Height() );
			const unsigned o   ( unsigned( it->NormalizedOpacity() * 255.0f ) );

			if (it->IsMaskVisible()) {
				bi.SetType ( BMM_GRAY_16 );
				bi.SetFlags( 0 );
				SmartInterface<Bitmap>    msk_bm( TheManager->Create( &bi ) );
				SmartPtr<BMM_Color_64>  line_ptr( new BMM_Color_64[ w ] );
				SmartPtr<WORD        > mline_ptr( new WORD        [ w ] );

				GetCOREInterface()->RenderTexmap( it->Mask(), msk_bm );

				for( int y = 0; y < h; ++y ) {
					msk_bm->Get16Gray( 0, y, w, mline_ptr );
					tex_bm->GetPixels( 0, y, w,  line_ptr );

					WORD        * mpix = mline_ptr;
					BMM_Color_64*  pix =  line_ptr;
					for( int x = 0; x < w; ++x, ++pix, ++mpix, data += 4 ) {
						const unsigned long alpha(  pix->a    );
						const unsigned long mask ( *mpix >> 8 );
				...
				for( int y = 0; y < h; ++y ) {
					...
					BMM_Color_64* pix = line_ptr;
					for( int x = 0; x < w; ++x, ++pix, data += 4 ) {
						const unsigned long alpha( pix->a );
						data[3] = (unsigned char)(alpha * o / 255 >> 8);
					}
				}
			}
...
void Composite::Texture::PrepareHWMultimap( TimeValue  time,
	...
	for (int i = 0; it != m_ViewportTextureList.end(); ++it, ++i) {
		...
		callback.GetGfxTexInfoFromTexmap( time, material->texture[0], this );
...
void Composite::Texture::PrepareSWMultimap( TimeValue  time,
	...
	for( int i = 0; it != m_ViewportTextureList.end(); ++i, ++it ) {
		callback.GetGfxTexInfoFromTexmap( time, material->texture[i], this );
