...
Reactor* FindReactor(ICurve* curve)
{
	...
	{
		if (dep.anims[x]->ClassID() == CURVE_CONTROL_CLASS_ID)
		{
			ReferenceTarget*cc = (ReferenceTarget*)dep.anims[x];
			MyEnumProc dep2(cc);
...
void ReactionDlg::ClearCurves()
{
	for (int i = 0; i < iCCtrl->GetNumCurves(); i++ )
	{
		...
		RefListItem* refItem = curve->GetRefList().first;
		while (refItem)
		{
			if (refItem->maker->ClassID() == CURVE_CONTROL_CLASS_ID)
			{
				ICurveCtl* cc = (ICurveCtl*)refItem->maker;
				if (cc != iCCtrl)
				{
...
void StateListItem::SetValue(float val)
{
	...
	else 
	{
		...
		if (r->type == REACTORFLOAT)
		{
			...
			RefListItem* refItem = r->GetRefList().first;
			while (refItem)
			{
				ReferenceMaker* maker = refItem->maker;
