...
bool AnimPopupMenu::Filter(PickObjectCallback* cb, Animatable *anim, bool includeChildren)
{
	bool valid = true;
	...
	for (int i = 0; i< anim->NumSubs(); i++)
	{
		if (anim->SubAnim(i))
		{
			valid = Filter(cb, anim->SubAnim(i));
		...
		for (int i = 0; i < parent->NumSubs(); i++)
		{
			...
			for (int p2 = 0; p2 < parents.Count(); p2++)
			{
				if (p2 != 0)
				{
					...
					{
						if (!_tcscmp(subAnimName, parents[p2]->SubAnimName(i2)))// if there are no differneces in the names
						{
							if (Filter(cb, parents[p2]))
							{
								match2 = true;
