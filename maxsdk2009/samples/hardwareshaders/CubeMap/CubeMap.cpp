...
MaxSDK::Util::Path CubeMapFileAccessor::GetPath() const	{
	return MaxSDK::Util::Path(mCubeMap->GetCubeMapFile());
}

void CubeMapFileAccessor::SetPath(const MaxSDK::Util::Path & aNewPath)	{
	mCubeMap->SetCubeMapFile(const_cast<TCHAR*>(aNewPath.GetCStr()));
}
...
MaxSDK::Util::Path CubeMapVertexAccessor::GetPath() const	{
	return MaxSDK::Util::Path(mCubeMap->GetVertexShaderFile());
}

void CubeMapVertexAccessor::SetPath(const MaxSDK::Util::Path& aNewPath)	{
...
HRESULT IDX9PixelShaderImp::Initialize(Material *mtl, INode *node)
{
	...
	if (map->GetInitPS()) {
		...
		MaxSDK::Util::Path cubeMapPath(map->GetCubeMapFile());
		bool found = 
			IPathConfigMgr::GetPathConfigMgr()->SearchForExternalFiles(cubeMapPath);
...
HRESULT IDX9PixelVertexShaderImp::LoadAndCompileShader()
{
		...
		HRESULT hr;
		MaxSDK::Util::Path vertexShaderPath(map->GetVertexShaderFile());
		bool found = IPathConfigMgr::GetPathConfigMgr()->
			SearchForExternalFiles(vertexShaderPath);
		...
		hr = D3DXAssembleShaderFromFile(vertexShaderPath.GetCStr(),NULL, NULL, 0L, &pCode, NULL);
