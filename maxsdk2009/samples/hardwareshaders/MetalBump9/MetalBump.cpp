...
// ip->SetUseEnvironmentMap(useEnvMap);
   ...
   if (success) {
      ...
      PBBitmap Bitmap;
		Bitmap.bi.SetName(originalPath);
      Bitmap.bm = NULL;
...
MaxSDK::Util::Path MaxShaderAccessor::GetPath() const {
   PBBitmap * bm;
   mParamBlock->GetValue(mID,0,bm,FOREVER);
   if(bm)   {
      return MaxSDK::Util::Path(bm->bi.Name());
   }
	else {
...
void MaxShaderAccessor::SetPath(const MaxSDK::Util::Path& aNewPath) {
   PBBitmap bm;

   bm.bi.SetName(aNewPath.GetCStr());
   bm.bm = NULL;
