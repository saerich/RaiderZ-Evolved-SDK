...
static void StripName(TCHAR* sname) {
	int n = static_cast<int>(_tcslen(sname)-1);	// SR DCAST64: Downcast to 2G limit.
...
//		return FALSE;
	int len = static_cast<int>(strlen(inbuf));	// SR DCAST64: Downcast to 2G limit.
	if(inbuf[len-1] == '\n')
		inbuf[len-1] = 0;
	theImport->pctcounter++;
	if(theImport->pctcounter > 100) {
...
static int face3d (void) {
	...
	if(dxf_type==DXF3D) {
		...
		if(v3.x==v4.x && v3.y==v4.y && v3.z==v4.z) {
			res = AddVerts(dxf_mesh,3);
			if(res < 0) {
				noram:
				object_overflow(res); 
				return(1);
				}
			res = AddFaces(dxf_mesh,1);
			if(res < 0)
				goto noram;
			...
			res = AddVerts(dxf_mesh,4);
			if(res < 0)
				goto noram;
			res = AddFaces(dxf_mesh,2);
			if(res < 0)
				goto noram;
...
static int MakeSolidArcObj(float astrt,float aend,short isCircle) {
	...
	float angstep,rad,t;
	Point3 pt;
	int i,nv1,nv2,nsteps,nverts,nstart,ncent1 = 0,ncent2 = 0;
...
static int AddPlineVert(Point3 p, float wstart) {
	...
	if (nplverts>1) {
		...
		if (nplverts>2||(!polyClosed)) {
			res = AddFaces(dxf_mesh,isThick?8:2);
			if (res < 0) { 
				object_overflow(res); 
				return(1); 
				}
...
//	please_wait(progstr(STR0560));
	...
	inbuf = (char *)inBuf.Alloc(MAXGTXT,TRUE);
	if(!inbuf)
		goto bailout;
	gtextv = (char *)gTextV.Alloc(MAXGTXT,TRUE);
	if(!gtextv)
		goto bailout;
	for (i = 0; i < 10; i++) {
		tgroup[i] = (char *)tGroup[i].Alloc(MAXGTXT,TRUE);
		if(!tgroup[i])
			goto bailout;
		}
	rgroup = (double *)rGroup.Alloc(60 * sizeof(double),TRUE);
	if(!rgroup)
		goto bailout;
	igroup = (short *)iGroup.Alloc(80 * sizeof(short),TRUE);
	if(!igroup)
		goto bailout;
