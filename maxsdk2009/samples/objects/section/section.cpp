...
#ifdef DEBUG_UPDATES
	static int inhere=0;
	DebugPrint("UpdateSlice #%04d - ",++inhere);
#endif
	if (!m_pleaseUpdate)
	{
		...
		if (m_update_method == UPDATE_SECT_MOVE)
		{
			...
			if ((m_justLoaded || EqualMatrices(curTM,m_sliceTM)) && !forceUpdate)
			{
				...
				m_sliceTM = curTM;
#ifdef DEBUG_UPDATES
				DebugPrint("curTM == lastTM (or just loaded)\n");
#endif
				return;
			}
		} 
...
#ifdef DEBUG_SLICE
				DebugPrint("face slice: segment #%d\t(%5.2f %5.2f %5.2f <--> %5.2f %5.2f %5.2f)\n",
					m_ptlist.cnt/2,
...
#ifdef DEBUG_SLICE
		DebugPrint("edge slice: segment #%d\t(%5.2f %5.2f %5.2f <--> %5.2f %5.2f %5.2f)\n",
				m_ptlist.cnt/2,
				m_facelines[jj].pts[0].p.x,
				m_facelines[jj].pts[0].p.y,
				m_facelines[jj].pts[0].p.z,
...
#ifdef DEBUG_SLICE
		DebugPrint("vertex slice: segment #%d\t(%5.2f %5.2f %5.2f <--> %5.2f %5.2f %5.2f)\n",
				m_ptlist.cnt/2,
				m_facelines[jj].pts[0].p.x,
				m_facelines[jj].pts[0].p.y,
				m_facelines[jj].pts[0].p.z,
...
#ifdef DEBUG_SLICE 
				DebugPrint("air slice: segment #%d\t(%5.2f %5.2f %5.2f <--> %5.2f %5.2f %5.2f)\n",
					m_ptlist.cnt/2,
					m_facelines[jj].pts[0].p.x,
					m_facelines[jj].pts[0].p.y,
					m_facelines[jj].pts[0].p.z,
...
void SectionObject::WeldSlice()
{
	...
	for (s1 = 0; s1 < shape.numLines; )
	{
...
#ifdef DEBUG_SLICE
		DebugPrint("seg #%d, s1p1.x-s1p2.x = %10.8f\n", s1, double(s1p1.x-s1p2.x));
		DebugPrint("         s1p1.y-s1p2.y = %10.8f\n", double(s1p1.y-s1p2.y));
#endif
		...
		if (WELD_SIGN(s1p1.x-s1p2.x)==0 && WELD_SIGN(s1p1.y-s1p2.y)==0)
		{
			shape.Delete(s1);
#ifdef DEBUG_WELD
			DebugPrint("section weld: removing null segment (#%d)\n",s1);
#endif
		}
	...
	for (s1 = 0; s1 < shape.numLines; ++s1)
	{
		...
		for (s2 = s1+1; s2 < shape.numLines; )
		{
			...
			if (p1p1_same && p2p2_same || p1p2_same && p2p1_same)
			{
				shape.Delete(s2);
#ifdef DEBUG_WELD
				DebugPrint("section weld: segment %d:  removing duplicate seg %d\n",s1,s2);
#endif
			}
			else
				++s2;
	...
	for (s1 = 0; s1 < shape.numLines-1; )
	{
		...
		for (s2 = s1+1; s2 < shape.numLines; )
		{
...
#ifdef DEBUG_WELD
			DebugPrint("lines[%d]: examining lines[%d]\n",s1,s2);
			shape.lines[s1].Dump();
#endif
			p1p1_same = WELD_SIGN(s1p1.x-s2p1.x)==0 && WELD_SIGN(s1p1.y-s2p1.y)==0;
			p1p2_same = WELD_SIGN(s1p1.x-s2p2.x)==0 && WELD_SIGN(s1p1.y-s2p2.y)==0;
			...
			if (p1p1_same)
			{
				...
				shape.Delete(s2);
#ifdef DEBUG_WELD
				DebugPrint("section weld: to segment %d, prepended segment %d pt2\n",s1,s2);
#endif
			}
			else if (p1p2_same)
			{
				...
				shape.Delete(s2);
#ifdef DEBUG_WELD
				DebugPrint("section weld: to segment %d, prepended segment %d pt1\n",s1,s2);
#endif
			}
			else if (p2p1_same)
			{
				...
				shape.Delete(s2);
#ifdef DEBUG_WELD
				DebugPrint("section weld: to segment %d, appended segment %d pt2\n",s1,s2);
#endif
			}
			else if (p2p2_same)
			{
				...
				shape.Delete(s2);
#ifdef DEBUG_WELD
				DebugPrint("section weld: to segment %d, appended segment %d pt1\n",s1,s2);
#endif
			}
			else
				++s2;
	...
	for (s1 = 0; s1 < shape.numLines; ++s1)
	{
		...
		{
#ifdef DEBUG_WELD
			DebugPrint("section weld: setting spline closed(#%d)\nhere's the pts before deleting last pt:\n",s1);
			shape.lines[s1].Dump();
#endif
		...
		else
		{
			shape.lines[s1].Open();
#ifdef DEBUG_WELD
			DebugPrint("section weld: setting spline open(#%d)\n",s1);
#endif
		}
	}
...
#ifdef DEBUG_MERGE
			//DebugPrint("weld colinear shape #%d, dump follows:\n", s1);
			//shape.lines[s1].Dump();
			DebugPrint("weld colinear checking shape #%d pts %d %d %d, v1.x-v2.x = %10.7f, v1.y-v2.y = %10.7f:\n", 
				s1,p1,p2,p3,v1.x - v2.x,v1.y - v2.y);
#endif
			if (WELD_SIGN(v1.x - v2.x) == 0 && WELD_SIGN(v1.y - v2.y) == 0)
			{
#ifdef DEBUG_MERGE
				DebugPrint("weld colinear shape #%d:  about to delete point %d\n", s1,p2);
#endif
...
int SectionObject::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags) 
{
	...
	//DebugPrint("in Display, cnt = %d\n",++cnt);
	
	ivalid = FOREVER;
	m_pblock->GetValue(PB_LENGTH, t, height, FOREVER);
	m_pblock->GetValue(PB_WIDTH, t, width, FOREVER);

	//DebugPrint("in Display, this = 0x%x height = %f, width = %f\n", (int)this, height, width);
...
{
#ifdef DEBUG_REFS
	DebugPrint("##SectObj Noti!  hTarget =0x%08x, message=0x%08x, partID=0x%08x\n",
