...
int GeneralLight::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags) 
{
	...
	if (inode->Selected())
		...
		if(useLight)
		{
			...
			//! If shadowing it will be displayed dull
			//! If not illuminating it look not so dull...
			//! TODO: This is still TBD - probably work wth Satuation - waiting for feedback.
			if(GetIViewportShadingMgr()->GetViewportShadingLimits()!=IViewportShadingMgr::kNone)
			{
				INodeShadingProperties * shadeProp = static_cast<INodeShadingProperties*>(inode->GetInterface(NODESHADINGPROPERTIES_INTERFACE));
				Color hsv = RGBtoHSV(Color(color.r,color.g,color.b));
				if(shadeProp->GetViewportIlluminate() && shadeProp->GetViewportShadowCasting())
				{

					hsv.g *=0.5f;
					hsv.b *=0.7f;
					color = HSVtoRGB(hsv);
//					color *= 0.6f;	//temp for Design feedback
				}
				else if(!shadeProp->GetViewportIlluminate())
				{
					hsv.g *=0.8f;
					hsv.b *=0.8f;
					color = HSVtoRGB(hsv);
//					color *= 0.8f;	//temp for Design feedback
