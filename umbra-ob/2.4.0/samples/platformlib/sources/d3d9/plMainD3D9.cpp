 ...
 *
 * \file
 * \brief     Entrypoint for Direct3D 9 Win32 applications.
 * 
 *//*-------------------------------------------------------------------*/
...
HWND InitInstance(const Platform::VideoMode &videoMode, HINSTANCE hInstance, int nCmdShow)
{
   ...
   RECT rc = {0, 0, videoMode.width, videoMode.height };
                     ...
                     int       nCmdShow)
{
    ...
    // TODO [sampo@umbrasoftware.com] re-write the message handling loop so that wParam has 0 in it
    // if the exit is clean. On windows, call PostQuitMessage(0);
