...
{
    PLATFORM_ASSERT(s > 0 && s <= MAX_SIZE);
    ...
    while (m_state.m_readDataAvailable < s)
    {
        ...
        PLATFORM_ASSERT(m_state.m_readDataAvailable >= s);
    }

    m_state.m_readDataCount += s;
    m_state.m_readDataAvailable -= s;

    PLATFORM_ASSERT(m_state.m_readDataAvailable >= 0);
...
void DataStreamReader::advance(int s)
{
    ...
    PLATFORM_ASSERT(m_state.m_readIndex < m_stream->m_dataSize);    
...
void DataStreamWriter::advance(int s)
{
    ...
    PLATFORM_ASSERT(m_state.m_writeIndex < m_stream->m_dataSize);    
