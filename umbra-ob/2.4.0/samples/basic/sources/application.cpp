...
const float     STAT_UPDATE_TIME    = 250.f;                        /*!< Frequency of stat updates on screen. */	
...
CONTROLNAME_FREEZE " - Freeze visibility\n"
"\n"
...
m_graphicsServices(NULL),
m_sampleCommander(NULL),
m_camera(NULL),
m_cell(NULL),
m_sphereModel(NULL),
m_debugLineMode(0),
...
/*-------------------------------------------------------------------*//*!
 *
 * \brief   Create and initialize application's Umbra::Camera, camera
 *			representation for Occlusion Booster.
 *
...
void Application::createCamera(void)
{
    ...
    m_camera->setFrustum(frustum);

    // Set Umbra:Camera to a Umbra::Cell.
    
    m_camera->setCell(m_cell);
	...
	m_camera->setCameraToCellMatrix((Umbra::Matrix4x4&)m_cameraToCell[0][0]);
}
...
void Application::umbraInit(void)
{
    ...
    // Create SampleGraphicsServices object
    
    SampleGraphicsServices* m_graphicsServices = new SampleGraphicsServices();
    assert(m_graphicsServices);

    // Create an Umbra::Commander object.
    
    m_sampleCommander = new SampleCommander(m_graphicsServices);
    assert(m_sampleCommander);
    ...
    // Create and initialize Umbra::Camera.
 ...
 *
 *          Releases Umbra::Cell and Umbra::Camera. Deletes 
 *          SampleCommander and SampleGraphicsServices. Calls
 *          Umbra::Library::exit.
 *
 *//*-------------------------------------------------------------------*/
...
{    
    ...
    // Release Umbra::Camera.

    m_camera->release();

    // Delete Umbra::Commander.

    delete m_sampleCommander;

    // Delete Umbra::LibraryDefs::GraphicsServices.
 ...
 *			Umbra::Camera with current frame's properties and camera 
 *			matrix.
 *
...
{    
    ...
    if (m_freeze)
		properties |= Umbra::Camera::FREEZE_OCCLUSION;

    // Set parameters for Umbra camera.
    
	m_camera->setProperties(properties);
...
void Application::createObjects(void)
{
    ...
    m_sphereModel = new Model(vertices, normals, triangles, vertexCount, triangleCount);
	...
	{       
        ...
        Object* obj = new Object(m_sphereModel);
        assert(obj);
...
void Application::animateObjects(void)
{
    Matrix4x4   objectToCell;
    Vector3     translation;
...
void Application::handleInput(void)
{
    ...
    if (((keyboardState.alphabetBitmap & Platform::KEY_F) &&
        !(m_keyboardState.alphabetBitmap & Platform::KEY_F)) ||
        ((padState.keyBitmap & Platform::BUTTON_L1) &&
        !(m_padState.keyBitmap & Platform::BUTTON_L1)))        
    {
        m_freeze = !m_freeze;
        SampleCommander::m_freeze = !SampleCommander::m_freeze;
    }
    ...
    {
		m_debugLineMode++;
		m_debugLineMode = m_debugLineMode % 6;
        ...
        Umbra::Library::clearFlags(Umbra::LibraryDefs::LINEDRAW, Umbra::LibraryDefs::LINE_CAMERA_FRUSTUM);
        ...
        else if (m_debugLineMode == 5)
			Umbra::Library::setFlags(Umbra::LibraryDefs::LINEDRAW, Umbra::LibraryDefs::LINE_CAMERA_FRUSTUM);
	}
 ...
 *
 *			The function performs two calls to Occlusion Booster. 
 *			Camera::resolveVisibility and Camera::processVisibility.
 *			The first prompts the library to start the visibility query.
 *			The latter indicates commander can be now invoked from the
 *			current thread.
 *
 *          In a single threaded renderer, Umbra Occlusion Booster
 *          callbacks are processes by the same thread which started the
 *          visibility query.
 *
 *//*-------------------------------------------------------------------*/
...
void Application::render(void)
{
    ...
    Platform::setViewport(0, 0, m_screenWidth, m_screenHeight);

    Platform::clear(0.25f, 0.25f, 0.25f, 0.f, 1.0f, 0.f);

    Matrix4x4 cellToCamera;
    cellToCamera.invert(m_cameraToCell);
    Platform::setViewMatrix(cellToCamera);

    // Call Umbra::Camera::resolveVisibility().
    // Occlusion booster will start determining which objects are visible.

    m_camera->resolveVisibility();

   	// Bind the recipient thread. Indicates that the thread is ready
	// to receive commander commands.
    //
    // In this case the recipient thread is the same as the thread calling 
	// Umbra::Camera::resolveVisibility. In a multi-threaded renderer the 
	// recipient thread would be the rendering thread, and Umbra::Camera::
	// resolveVisibility could be called from e.g. the game thread after 
	// all updates have been made. 

    m_camera->processVisibility(m_sampleCommander);
...
void Application::renderStats(void)
{
	...
	Platform::fontPrint(5, 5, Vector4(1,1,1,1), 
		"%g ms\n"
		"%d fps\n"
		"Occlusion culling: %s\n"
		"Latent occlusion: %s\n"
...
{
	if(m_helpDisplayed)
		Platform::fontPrint(m_helpTextX, m_helpTextY, Vector4(1,1,1,1), g_onscreenHelpText);
	else
		Platform::fontPrint(5, m_helpPromptY, Vector4(1,1,1,1), g_onscreenHelpPrompt);
}
