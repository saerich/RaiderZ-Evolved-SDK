...
Model::Model(Vector3* vertices, Vector3* normals, Vector3i* triangles, int vertexCount, int triangleCount, Renderer* renderer) :
m_umbraModel(NULL),
m_platformMesh(NULL),
m_renderer(renderer)
{
    ...
    m_platformMesh  = m_renderer->createMesh(vertices, normals, triangles, vertexCount, triangleCount);
...
Model::~Model(void)
{
    m_renderer->deleteMesh(m_platformMesh);
