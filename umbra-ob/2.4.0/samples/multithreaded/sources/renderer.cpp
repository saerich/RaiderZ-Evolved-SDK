...
/*-------------------------------------------------------------------*//*!
 * \brief	Creates a platform mesh from mesh data.
 *//*-------------------------------------------------------------------*/

Platform::Mesh* Renderer::createMesh(const Vector3* vertices, const Vector3* normals, const Vector3i* triangles, int nVertices, int nTriangles)
{
    Platform::Mesh* mesh = NULL;
    
    m_dataStreamWriter.put(CMD_CREATE_MESH);
    m_dataStreamWriter.put((void**)&mesh);
    m_dataStreamWriter.put((void*)vertices);
    m_dataStreamWriter.put((void*)normals);
    m_dataStreamWriter.put((void*)triangles);
    m_dataStreamWriter.put(nVertices);
    m_dataStreamWriter.put(nTriangles);

    // Wait for the rendering thread to complete all commands.
    finish();
...
/*-------------------------------------------------------------------*//*!
 * \brief	Destroys a platform mesh. 
 *//*-------------------------------------------------------------------*/

void Renderer::deleteMesh(Platform::Mesh* mesh)
{
    m_dataStreamWriter.put(CMD_DELETE_MESH);
    m_dataStreamWriter.put((void*)mesh);
}
