...
{
public:
    static ID3D10Device*                getDevice(void)                 { assert(s_device); return s_device; }
    static void                         setDevice(ID3D10Device* d)      { s_device = d; }

    static ID3D10RenderTargetView*      getRenderTargetView(void)                       { assert(s_renderTargetView); return s_renderTargetView; }
    static void                         setRenderTargetView(ID3D10RenderTargetView* d)  { s_renderTargetView = d; }

    static IDXGISwapChain*              getSwapChain(void)                 { assert(s_swapChain); return s_swapChain; }
...
public:
      ...
      m_numVertices (nVertices)
    {
        ...
        for(int i = 0; i < nump; i++)
        {
            ...
            assert((triangles[i][0]) < 65536 && (triangles[i][1] < 65536) && (triangles[i][2] < 65536));
...
bool Platform::init(const VideoMode &mode, void* p)
{
	...
	} else
	{
		...
		sampleCount = 1;
		assert("Unsupported multisample mode" && false);
		return false;
	}
...
static void D3D10drawUP(const Vector3* vertices, int vertexCount, const unsigned int* indices, int indexCount, D3D10_PRIMITIVE_TOPOLOGY topology)
{
	...
	{
		assert("drawIndexedUP: vertex or index buffer not big enough to hold user data" && false);
		return;
	}
	...
	if(rv != S_OK)
	{
		assert("drawIndexedUP: couldn't map vertex buffer" && false);
		return;
	}
	...
	if(useIndices)
	{
		...
		if(rv != S_OK)
		{
			assert("drawIndexedUP: couldn't map index buffer" && false);
			return;
		}
