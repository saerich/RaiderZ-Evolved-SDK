...
const tSigned32 PATHENGINE_INTERFACE_MINOR_VERSION = 8;
...
public:
    ...
    virtual iMesh* buildTileMeshFromContent(tSigned32 tileIndex, const iContentChunkInstance* const* instancePointers, tSigned32 numberOfInstances, const char *const* options, iOutputStream* os) const = 0;
    virtual tSigned32 getNumberOfGroundTilesOverlapped(tSigned32 tileIndex, const iSourceTiling* tiling) const = 0;
    virtual void getGroundTilesOverlapped(tSigned32 tileIndex, const iSourceTiling* tiling, tSigned32* resultsBuffer, tSigned32& numberOverlapped) const = 0;
    virtual iMesh* buildTileMeshFromLocalGroundTiles(tSigned32 tileIndex, const iSourceTiling* tiling, const iMesh* const* meshPointers, tSigned32 numberOfMeshes, const char *const* options) const = 0;
    void operator delete(void* voidPointer)
    {
        if(voidPointer)
        {
...
public:
    ...
    virtual void testJumpCandidatePair(const tSigned32* fromPoint3D, const tSigned32* toPoint3D, tSigned32& penaltyOut, tSigned32& penaltyBack) = 0;    
    virtual void testDropCandidatePair(const tSigned32* fromPoint3D, const tSigned32* toPoint3D, tSigned32& penaltyOut, tSigned32& penaltyBack) = 0;    
};
