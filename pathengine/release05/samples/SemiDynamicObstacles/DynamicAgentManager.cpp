...
cDynamicAgentManager::cDynamicAgentManager(iMesh* mesh, iShape* agentShape, const iCollisionContext* context, tSigned32 numberOfAgents) :
 _mesh(mesh),
 ...
 _regions(numberOfAgents, -1)
{
    ...
    _shapeRenderPreprocess = gTestBed->preprocessShapeWithHeight(_agentShape, 60);
}
cDynamicAgentManager::~cDynamicAgentManager()
{
    ...
    gTestBed->deleteRenderPreprocess(_shapeRenderPreprocess);
}
...
cDynamicAgentManager::renderAgents() const
{
    ...
    for(i = 0; i != SizeL(_agents); ++i)
    {
        gTestBed->drawPreprocessedGeometryAtPosition(_shapeRenderPreprocess, _agents[i]->getPosition());
    }
}
void
cDynamicAgentManager::renderAgents_ColouredByRegion() const
{
    ...
    for(i = 0; i != SizeL(_agents); ++i)
    {
        ...
        {
            gTestBed->setColour(colours[_regions[i] % numberOfColours]);
        }
        gTestBed->drawPreprocessedGeometryAtPosition(_shapeRenderPreprocess, _agents[i]->getPosition());
    }
}
