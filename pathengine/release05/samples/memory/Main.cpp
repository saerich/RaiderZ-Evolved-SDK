        ...
        )
{
    ...
    mesh->generatePathfindPreprocessFor(shape, pathfindPreprocessAttributes);
...
// check if interface for dll loaded is compatible with the headers used for compilation
    ...
    {
        cErrorHandler::refInstance().handle("Fatal", "The pathengine dll loaded is not built with memory tracking enabled.",0);
        return 0;
    }
