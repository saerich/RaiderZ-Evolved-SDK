...
// check if interface for dll loaded is compatible with the headers used for compilation
...

    {
        ...
        for(i = 0; i < agentShapes.size(); ++i)
        {
            ...
            mesh->generateCollisionPreprocessFor(agentShapes[i], 0);
            mesh->generatePathfindPreprocessFor(agentShapes[i], attributes);
        }
    ...
    RunThreadPoolAndMeasureThroughput(2, globals);
