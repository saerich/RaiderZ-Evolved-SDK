...
//
// Originally written by: Matthias M?ler-Fischer (03-08-05)
//
// ===============================================================================
...
// ------------------------------------------------------------------------------------
void RefreshDisplayString(char *demoName, bool bTearDemo=false)
{
...
void ProcessKeys()
{
	...
	{	
		...
		switch (i)
		{
			...
			case 'z': g_Move -= 0.1f; break;
			case 'x': g_Move += 0.1f; break;
...
void SetupCurtainScene()
{
	...
	NxActor *box1 = CreateBox(NxVec3(1,1.5,0), NxVec3(1,1,2), 10);
	...
	g_pBox = box1;
	box1->raiseBodyFlag(NX_BF_DISABLE_GRAVITY);
	box1->raiseBodyFlag(NX_BF_FROZEN);
...
//	clothDesc.flags &= ~NX_CLF_GRAVITY;
	...
	MyCloth *regularCloth = new MyCloth(gScene, clothDesc, 4.5f, 4.5f, 0.15f, "ageia.bmp");
	if (!regularCloth->getNxCloth())
	{
		printf("Error: Unable to create the cloth for the current scene.\n");
		delete regularCloth;
...
bool InitNx()
{
	...
	switch(gSceneNr) {
		...
		case 9 : SetupRemapMeshScene(); break;
	}
...
void RunPhysics()
{
	...
	g_pBox->setGlobalPosition(NxVec3(1,1.5,g_Move));
