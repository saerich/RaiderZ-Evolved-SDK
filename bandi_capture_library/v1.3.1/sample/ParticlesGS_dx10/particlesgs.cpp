...
BOOL DoCapture(BOOL bCapture)
{
	...
	if(bCapture == TRUE)
	{
		...
		// start capture
		hr = g_bandiCaptureLibrary.Start(szCaptureFilePathName, NULL, BCAP_MODE_D3D10, 
										(LONG_PTR)DXUTGetDXGISwapChain());
		if(FAILED(hr))
		{
			ASSERT(0);
			g_bandiCaptureLibrary.Stop();
...
void InitApp()
{
	...
	MessageBox(NULL, _T("BCL Sample for DirectX10.\n\n") 
...
#endif
	if(uMsg == WM_KEYDOWN && wParam == VK_SCROLL)
                                      ...
                                      void* pUserContext )
{
    ...
    WCHAR str[MAX_PATH];
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"data\\ParticlesGS.fx" ) );
    ...
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"data\\Particle.dds" ) );
