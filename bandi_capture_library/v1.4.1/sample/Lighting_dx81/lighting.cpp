...
CMyD3DApplication::CMyD3DApplication()
{
	...
	MessageBox(NULL, _T("BCL Sample for DirectX8.\n\n") 
...
HRESULT CMyD3DApplication::Render()
{
	...
	{
		g_bandiCaptureLibrary.Work((LONG_PTR)m_pd3dDevice);
		// 캡처중임을 알리는 메시지 출력

		if( SUCCEEDED( m_pd3dDevice->BeginScene() ) )
		{
			m_pFont->DrawText( 2, 60, D3DCOLOR_ARGB(255,255,0,0), TEXT("REC"));
...
// 캡처 하기
BOOL CMyD3DApplication::DoCapture(BOOL bCapture)
{
	...
	if(bCapture == TRUE)
	{
		...
		// start capture
		hr = g_bandiCaptureLibrary.Start(szCaptureFilePathName, NULL, BCAP_MODE_D3D8, (LONG_PTR)m_pd3dDevice);
		if(FAILED(hr))
		{
			ASSERT(0);
			g_bandiCaptureLibrary.Stop();
                                  ...
                                  LPARAM lParam )
{
    ...
    {
		case WM_KEYDOWN:
			if(wParam == VK_SCROLL || wParam == VK_F12)
