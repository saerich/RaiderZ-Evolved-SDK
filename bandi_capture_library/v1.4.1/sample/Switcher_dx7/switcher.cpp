...
BOOL DoCapture(BOOL bCapture)
{
	...
	if(bCapture == TRUE)
	{
		...
		// start capture
		hr = g_bandiCaptureLibrary.Start(szCaptureFilePathName, NULL, BCAP_MODE_DDRAW7, (LONG_PTR)g_pDDSBack);
		if(FAILED(hr))
		{
			ASSERT(0);
			g_bandiCaptureLibrary.Stop();
...
BOOL DoCapture_Jpeg()
{
	...
	// get (target directory + filename by date)
	TCHAR szCaptureFilePathName[MAX_PATH*2];
	g_bandiCaptureLibrary.MakePathnameByDate(_T("c:\\"), _T("Capture"), _T("jpg"),  szCaptureFilePathName, MAX_PATH*2);

	if(FAILED(g_bandiCaptureLibrary.CaptureImage(szCaptureFilePathName, BCAP_IMAGE_JPG, 80, BCAP_MODE_DDRAW7, TRUE, (LONG_PTR)g_pDDSBack)))
		return FALSE;
...
UpdateFrame(HWND hWnd)
{
	...
	{
		g_bandiCaptureLibrary.Work((LONG_PTR)g_pDDSBack);
		if (g_pDDSBack->GetDC(&hDC) == DD_OK)
		{
			// Paint the bouncing ball
        ...
        case WM_KEYDOWN:
        	...
        	switch (wParam)
        	{
            	...
            	case VK_F12:
                    ...
                    return 0L;
				case VK_SCROLL:
					if(DoCapture(!g_bandiCaptureLibrary.IsCapturing()) == TRUE)
						DoCapture_Jpeg();
					break;
        	}
        	break;
        ...
        int         nCmdShow)
{
	...
	MessageBox(NULL, _T("BCL Sample for DirectX7.\n\n") 
