...
CCaptureTest* g_captureTest=NULL;
...
INT WINAPI WinMain( HINSTANCE hInstance, HINSTANCE, LPSTR, int )
{
	...
	g_captureTest = new CCaptureTest();
...
#if _WIN64
    DXUTCreateWindow( L"BandiCapSample (64bit)" );
#else
	DXUTCreateWindow( L"BandiCapSample" );
#endif
	...
	// init capture main
	g_captureTest->Init(hInstance, DXUTGetHWND());
...
void InitializeDialogs()
{
	...
	if(g_captureTest)
		g_captureTest->InitializeDialogs(iY);
}
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
{
	...
	*/
	...
	if(g_captureTest)
		g_captureTest->OnD3D9CreateDevice(pd3dDevice, pBackBufferSurfaceDesc );
                                ...
                                const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
{
    ...
    *(LPDWORD)lr.pBits = D3DCOLOR_RGBA( 255, 255, 255, 255 );
	...
	if(g_captureTest)
		g_captureTest->OnD3D9ResetDevice(pd3dDevice, pBackBufferSurfaceDesc);
...
void CALLBACK OnFrameMove( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
{
	...
	if(g_captureTest)
		g_captureTest->OnFrameMove(fTime, fElapsedTime );
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
{
	...
	// 캡처
	if(g_captureTest)
		g_captureTest->CaptureScene(pd3dDevice);

	// 캡처후 랜더 처리
	// 지금부터 랜더되는 오브젝트는 캡처된 동영상에 표시 되지 않음
    // Begin the scene
    if( g_captureTest && SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
		...
		g_captureTest->RenderText(txtHelper, FALSE);
		txtHelper.End();

		g_captureTest->RenderScene(pd3dDevice);
...
void RenderText()
{
	...
	g_captureTest->RenderText(txtHelper, TRUE);
...
LRESULT CALLBACK MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, bool* pbNoFurtherProcessing )
{
    *pbNoFurtherProcessing = g_HUD.MsgProc( hWnd, uMsg, wParam, lParam );
	...
	if(g_captureTest)
		g_captureTest->MsgProc(hWnd, uMsg, wParam, lParam,pbNoFurtherProcessing);
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl )
{
	...
	default :	
		if(g_captureTest)
			g_captureTest->OnGUIEvent(nEvent, nControlID, pControl);
		break;
    }
}
...
void CALLBACK OnLostDevice()
{
	...
	if(g_captureTest)
		g_captureTest->OnD3D9LostDevice();
}
...
void CALLBACK OnDestroyDevice()
{
	...
	if(g_captureTest)
		g_captureTest->OnD3D9DestroyDevice();
}
