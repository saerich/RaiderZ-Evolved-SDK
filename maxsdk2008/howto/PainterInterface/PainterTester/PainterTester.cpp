...
static INT_PTR CALLBACK PainterTesterDlgProc(
...
int PainterTester::LoadCustomNodes()
{
...
BOOL  PainterTester::PaintStroke(BOOL hit, IPoint2 mousePos, 
	...
	{
	DebugPrint("face inde %d \n", index);


	if (index < faceData.Count())
	{
		DebugPrint("patch index %d \n", faceData[index].owningPatch);
		Point2 uvw = (faceData[index].st[0] * bary.x) + 
					 (faceData[index].st[1] * bary.y) + 
					 (faceData[index].st[2] * bary.z);
		DebugPrint("patch uv %f %f %f \n", uvw.x,uvw.y);
	}
