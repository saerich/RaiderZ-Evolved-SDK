...
static INT_PTR CALLBACK  [!output CLASS_NAME]DlgProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
	[!output CLASS_NAME]Dlg *theDlg;
	if (msg == WM_INITDIALOG) {
		theDlg = ([!output CLASS_NAME]Dlg*)lParam;
		DLSetWindowLongPtr(hwndDlg, lParam);
	} else {
	    if ( (theDlg = DLGetWindowLongPtr<[!output CLASS_NAME]Dlg *>(hwndDlg) ) == NULL )
			return 0; 
	}
	theDlg->isActive = 1;
...
INT_PTR CALLBACK HiliteWndProc( HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam ) 
{
...
[!output CLASS_NAME]Dlg::[!output CLASS_NAME]Dlg( HWND hwMtlEdit, IMtlParams *pParams)
{
	...
	for( int i = 0; i < SHADER_NCOLBOX; ++i )
		cs[ i ] = NULL;
...
INT_PTR [!output CLASS_NAME]Dlg::PanelProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam ) 
{
