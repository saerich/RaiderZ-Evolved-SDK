...
void [!output CLASS_NAME]::BeginEditParams( IObjParam *ip, ULONG flags, Animatable* prev )
{
	...
	NotifyDependents(Interval(t,t), static_cast<PartID>(PART_ALL), REFMSG_BEGIN_EDIT);
	NotifyDependents(Interval(t,t), static_cast<PartID>(PART_ALL), REFMSG_MOD_DISPLAY_ON);
...
void [!output CLASS_NAME]::EndEditParams( IObjParam *ip, ULONG flags, Animatable *next)
{
	...
	NotifyDependents(Interval(t,t), static_cast<PartID>(PART_ALL), REFMSG_END_EDIT);
	NotifyDependents(Interval(t,t), static_cast<PartID>(PART_ALL), REFMSG_MOD_DISPLAY_OFF);
	this->ip = NULL;
}
