...
TSTR FindFile(TCHAR *File)
{

	IPathConfigMgr* configManager = IPathConfigMgr::GetPathConfigMgr();
	MaxSDK::Util::Path result(File);

	// standard max search for external assets
	if(configManager->SearchForExternalFiles(result))	{
