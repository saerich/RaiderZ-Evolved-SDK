...
#ifndef SUPRESS_DXMATERIAL_DLL_MISSING_MESSAGE
	dd->LoadADll(filename,TRUE);
#else // SUPRESS_DXMATERIAL_DLL_MISSING_MESSAGE
	HANDLE hFile = CreateFile(filename, 0, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if (hFile != INVALID_HANDLE_VALUE) {
		CloseHandle(hFile);
		dd->LoadADll(filename,TRUE);
	}
#endif // SUPRESS_DXMATERIAL_DLL_MISSING_MESSAGE
...
#ifndef NO_MTL_DIRECTX_9_SHADER
			_tcscat(filename,GetString(IDS_DX_MATERIAL));
			dd->LoadADll(filename,TRUE);
#endif	// no_mtl_directx_9_shader
