...
bool ViewportLoader::IsSaveFxSupported()
{
	...
	{
		// if it is directX shading or plugin effect shading now, return true.
		return this->IsDxStdMtlEnabled() || this->IsCurrentEffectEnabled();
	}
	else
	{
		return false;
...
BOOL DXEffectActions::IsEnabled(FunctionID actionID)
{
	...
	{
		IViewportShaderManager3* pViewportLoader = (IViewportShaderManager3*)(
			FindCustAttribInterface(
			mtl, VIEWPORT_SHADER_MANAGER_INTERFACE3));
