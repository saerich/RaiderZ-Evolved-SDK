...
SunMaster::SunMaster(bool daylight) {
	...
	if (daylightSystem)
		ReplaceReference( REF_PARAM, CreateParameterBlock( desc3Daylight, 9, SUNMASTER_VERSION_DAYLIGHT ) );
	else
    ...
    CityList::Entry * pEntry = cityList.GetDefaultCity();
					...
					compassObj->NotifyDependents(FOREVER, PART_OBJ, REFMSG_CHANGE);
						...
						INode* newNode;
							...
							// Create the NaturalLightAssembly
										 newNode = NatLightAssembly::CreateAssembly(createInterface, natLightObj);
							assert(natLightObj);
							assert (newNode);
