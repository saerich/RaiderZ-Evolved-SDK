...
void LocationDialog::initMapList(HWND h)
{
	...
	CityList::Entry * defaultCity = cityList.GetDefaultCity();
	if (defaultCity != NULL)
	{
	...
	if (defaultCityLatitude != 0.0f && defaultCityLongitude != 0.0f)
	{
		for (int j=0; j<=LAST_MAP_NAME; ++j)
		{
			...
			if (hMap != INVALID_HANDLE_VALUE)
			{
				...
				if (  (-defaultCityLongitude > (longitude - width / 2.)) //signs are reversed for longitude
					&&(-defaultCityLongitude < (longitude + width / 2.)) //signs are reversed for longitude
