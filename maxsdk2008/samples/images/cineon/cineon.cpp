...
ffl(unsigned int x)
{
 ...
 // clamp to finite numbers
    ...
    for (int i = 0; i < 24; i++)
        if ((mantissa << i) & 0x400000) fraction += 1.0f / (float) pow(2.0f, i+1);

    return (float) (sign * fraction * pow(2.0f,exp));
}
...
CineonFile::ColorCorrect(U16& pixel , int n) {
    ...
    R32     desityStep  = CALIBRATED_DENSITY / (float) pow(2.0f, GetPixelSize());
    ...
    linearExposure  = (double) SIXTEEN_BIT_MAX * pow(10.0, logExposure);
...
CineonFile::GetScanLine(U16* line, U32 lineNum, U32 lineLen)
{
	...
	CINLine = (U8*) calloc(cellsPL, cellSize);
    if (CINLine == NULL) {
        return FALSE;
    }
...
{
    R32     mSoftclip   = 0.0f;
    U32     LUTsize     = (U32) pow(2.0f, bitsPerPixel);
    ...
    else
    {
        ...
        double Gain = SIXTEEN_BIT_MAX / (1- pow(10.0, (mRefBlack - mRefWhite) * densityStep/0.6 ));
        ...
        for (U32 cnt = 0; cnt < LUTsize; cnt++) 
        {
            ...
            dDensityLUT = pow(10.0,(cnt-mRefWhite)*densityStep/0.6)*Gain - Offset;
...
CineonFile::SetLog(U8 bitsPerPixel)
{
    ...
    U32     LUTsize;
    double  logDensity;
    double  densityMAX  = (float) log10(static_cast<float>(SIXTEEN_BIT_MAX));
    R32     densityStep  = CALIBRATED_DENSITY / (float) pow(2.0f, bitsPerPixel);

    if (mLogLUT) free(mLogLUT);

    LUTsize     = (U32) pow(2.0, 16);
	mLogLUT = (U16*) calloc(LUTsize, sizeof(U16));
    if (mLogLUT == NULL) {
        return FALSE;
    }
