...
void	UnwrapMod::ShrinkSelectUVEdge()
{
	...
	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		...
		for (int i = 0; i < edgeCount; i++)
		{
			if ( (esel[i])  && (ld->GetTVEdgeNumberTVFaces(i)/*TVMaps.ePtrList[i]->faceList.Count()*/ == 1) )
			{
				int a = ld->GetTVEdgeVert(i,0);//TTVMaps.ePtrList[i]->a;
				int b = ld->GetTVEdgeVert(i,0);//TTVMaps.ePtrList[i]->b;
				if ( (edgeCounts[a] == 1) || (edgeCounts[b] == 1) )
					esel.Set(i,FALSE);
			}
		}
	...
	else if (fnGetTVSubMode() == TVEDGEMODE)
	{
		...
		for (int i = 0; i < ld->GetNumberFaces()/*TVMaps.f.Count()*/; i++)
		{
			...
			for (int j = 0; j < deg; j++)
			{
				...
				{
					if (ld->GetGeomEdgeSelected(gIndex)/*gesel[gIndex]*/)
					{
						int tva,tvb;
						tva = ld->GetTVEdgeVert(tIndex,0);//TVMaps.ePtrList[tIndex]->a;
						tvb = ld->GetTVEdgeVert(tIndex,1);//TVMaps.ePtrList[tIndex]->b;

						if (ld->GetTVVertFrozen(tva) || ld->GetTVVertFrozen(tvb))
//							(TVMaps.v[tva].flags & FLAG_FROZEN) ||
//							(TVMaps.v[tvb].flags & FLAG_FROZEN) )
						{
							ld->SetTVEdgeSelected(tIndex,FALSE);
							ld->SetGeomEdgeSelected(gIndex,FALSE);
//							esel.Set(tIndex,FALSE);
//							gesel.Set(gIndex,FALSE);
						}
						else 
							ld->SetTVEdgeSelected(tIndex,TRUE);
