...
//TV VERTEX METHODS********************************************
	...
	int AddTVVert(TimeValue t,Point3 p, UnwrapMod *mod);
	...
	int AddTVVert(TimeValue t, Point3 p, int faceIndex, int ithVert, UnwrapMod *mod, BOOL sel = FALSE);
	//same as above but for a patch handle
	int AddTVHandle(TimeValue t,Point3 p, int faceIndex, int ithVert, UnwrapMod *mod, BOOL sel = FALSE);
	//same as above but for a patch interior
	int AddTVInterior(TimeValue t,Point3 p, int faceIndex, int ithVert, UnwrapMod *mod, BOOL sel = FALSE);
...
//several relax algorithms
	...
	void  RelaxVerts2(int subobjectMode, float relax, int iter, BOOL boundary, BOOL saddle, UnwrapMod *mod);
	
	//this a relax that uses the shape of the geo faces to align texture faces
	void RelaxByFaceAngle(int subobjectMode, int frames, float stretch, float str, BOOL lockEdges,HWND statusHWND, UnwrapMod *mod);

	//this a relax algorithm based on the angles of edges that touch a vertex
	void RelaxByEdgeAngle(int subobjectMode, int frames, float stretch,float str, BOOL lockEdges,HWND statusHWND, UnwrapMod *mod);
