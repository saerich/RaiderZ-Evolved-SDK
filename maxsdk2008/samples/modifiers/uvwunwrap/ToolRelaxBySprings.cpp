...

{
	...
	{
		mMeshTopoData[ldID]->RelaxByFaceAngle(fnGetTVSubMode(), frames, stretch, str, lockEdges, statusHWND, this);
	}
...

{
	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		mMeshTopoData[ldID]->RelaxByEdgeAngle(fnGetTVSubMode(), frames, stretch, str, lockEdges, statusHWND, this);
	}
