...
void  MeshTopoData::RelaxVerts2(int subobjectMode, float relax, int iter, BOOL boundary,BOOL saddle, UnwrapMod *mod)
      ...
      for (int i = 0; i < TVMaps.v.Count(); i++)
         {
			 ...
			 p.y = rmesh->verts[i].y;
			 SetTVVert(t,i,p,mod);
//         TVMaps.v[i].p.x = rmesh->verts[i].x;
  //       TVMaps.v[i].p.y = rmesh->verts[i].y;
    //     if (TVMaps.cont[i]) 
      ...
      for (int i = 0; i < TVMaps.v.Count(); i++)
         {
			 ...
			 p.y = rmnMesh->P(i).y;
			 SetTVVert(t,i,p,mod);
//         TVMaps.v[i].p.x = rmnMesh->P(i).x;
  //       TVMaps.v[i].p.y = rmnMesh->P(i).y;
    //     if (TVMaps.cont[i]) 
	//		 TVMaps.cont[i]->SetValue(t,&TVMaps.v[i].p);
         }
...
void MeshTopoData::RelaxByFaceAngle(int subobjectMode, int frames, float stretch, float str, BOOL lockEdges,HWND statusHWND, UnwrapMod *mod)
	...
	if (ct)
	{
		...
		for (int cframe = 0; cframe < frames; cframe++)
		{
			...
			for (int i = 0; i < TVMaps.v.Count(); i++)
			{
				...
				if ((w > 0.0f) && (!(TVMaps.v[i].IsFrozen())) && !isRigPoint)//	(!rigPoint[i])))
				{
					if (lockEdges)
					{
...
void MeshTopoData::RelaxByEdgeAngle(int subobjectMode, int frames, float stretch,float str, BOOL lockEdges,HWND statusHWND, UnwrapMod *mod)
