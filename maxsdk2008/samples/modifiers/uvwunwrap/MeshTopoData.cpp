...
int MeshTopoData::AddTVVert(TimeValue t,Point3 p, UnwrapMod *mod)
{
...
int MeshTopoData::AddTVVert(TimeValue t, Point3 p, int faceIndex, int ithVert, UnwrapMod *mod,  BOOL sel)
{
...
int MeshTopoData::AddTVHandle(TimeValue t, Point3 p, int faceIndex, int ithVert, UnwrapMod *mod,  BOOL sel)
{
	if ((faceIndex < 0) || (faceIndex >= GetNumberFaces()))
	{
		DbgAssert(0);
...
int MeshTopoData::AddTVInterior(TimeValue t, Point3 p, int faceIndex, int ithVert, UnwrapMod *mod,  BOOL sel)
{
	if ((faceIndex < 0) || (faceIndex >= GetNumberFaces()))
	{
		DbgAssert(0);
...
void MeshTopoData::Stitch(BOOL bAlign, float fBias, BOOL bScale, UnwrapMod *mod)
{
	...
	if (mVSel.NumberSet() == 1)
	{
		...
		connectedVerts.SetSize(mVSel.GetSize());
		...
		{        
			...
			if ( mVertexClusterList[a] == clusterId)
			{
				potentialVertex = a;
				potentialVertexOp = b;
			}
			if ( mVertexClusterList[b] == clusterId)
			{
				potentialVertex = b;
				potentialVertexOp = a;
			}
...
void MeshTopoData::BuildSpringData(Tab<EdgeBondage> &springEdges, Tab<SpringClass> &verts, Point3 &rigCenter,Tab<RigPoint> &rigPoints, Tab<Point3> &initialPointData, float rigStrength)
{
		...
		if (seedVert != -1)
		{
			...
			while ((nextVert != seedVert) && (!bail))
			{
				...
				else
				{
					...
					tempVerts.Set(currentVert,FALSE);
					Point3 a, b;
					a = tvData->v[currentVert].GetP();
					b = tvData->v[prevVert].GetP();
					d += Length(a-b);					
	...
	while (!done)
	{
		...
		Point3 a = tvData->v[prevVert].GetP();
		Point3 b = tvData->v[currentVert].GetP();
		...
		if (startVert != prevVert)
		{
			...
			rp.d = Length(rp.p - tvData->v[prevVert].GetP());
			rp.index = prevVert;
			...
			sp.edgePos = rp.p;
			sp.edgeIndex = -1;
			sp.originalVertPos = tvData->v[prevVert].GetP();
