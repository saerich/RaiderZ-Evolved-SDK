...
void CreateFaceMouseProcTemplate::DrawCreatingFace(HWND in_hwnd) {
	...
	MoveToEx(hdc, m_points[0].m_screenLocation.x, m_points[0].m_screenLocation.y, NULL);
	LineTo(hdc, m_mouseLocation.x, m_mouseLocation.y);
	if (m_points.length() > 1) LineTo (hdc, m_points.last().m_screenLocation.x, m_points.last().m_screenLocation.y);
