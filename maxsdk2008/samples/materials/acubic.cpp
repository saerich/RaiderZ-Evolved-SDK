...
MaxSDK::Util::Path ACubicPathAccessor::GetPath() const	{
	return MaxSDK::Util::Path(mBlock->GetStr(acubic_bitmap_names, 0, mTabIndex));
}

void ACubicPathAccessor::SetPath(const MaxSDK::Util::Path& aNewPath)	{
	mBlock->SetValue(acubic_bitmap_names, 0, const_cast<TCHAR*>(aNewPath.GetCStr()), mTabIndex);
}
