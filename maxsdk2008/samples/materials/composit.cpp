...
static INT_PTR CALLBACK PanelDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) 
   {
   CompositeDlg *theDlg = DLGetWindowLongPtr<CompositeDlg*>(hWnd);
...
INT_PTR CompositeDlg::PanelProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam ) 
   {
...
void CompositeDlg::LoadDialog(BOOL draw) 
   ...
   if (theTex) {     
      ...
      int i;
      for (i=0; i<min(theTex->subTex.Count(),NDLG); i++) {
