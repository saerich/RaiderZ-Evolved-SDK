...
{
    MrBlueObject *th = DLGetWindowLongPtr<MrBlueObject *>( hDlg); 
...
AnimObjPick::Pick(IObjParam *ip,ViewExp *vpt)
{
    ...
    if ((node = vpt->GetClosestHit()) != NULL) {
        ...
        MrBlueAnimObj *aobj = new MrBlueAnimObj();
        int id = obj->animObjects.Append(1, &aobj);

        RefResult ret = obj->ReplaceReference( id + 1, node);
...
MrBlueObject::BeginEditParams( IObjParam *ip, ULONG flags,Animatable *prev )
{
    ...
    if ( !hMrBlueActions ) {
        ...
        if (hMrBlueActions)
            DLSetWindowLongPtr( hMrBlueActions, this);
        if (hMrBlueTriggers)
            DLSetWindowLongPtr( hMrBlueTriggers, this);
        if (hMrBlueParamsHL)
            DLSetWindowLongPtr( hMrBlueParamsHL, this);
        if (hMrBlueParamsVPT)
            DLSetWindowLongPtr( hMrBlueParamsVPT, this);
        if (hMrBlueParamsAnim)
            DLSetWindowLongPtr( hMrBlueParamsAnim, this);
...
MrBlueObject::EndEditParams( IObjParam *ip, ULONG flags,Animatable *prev)
{
   ...
   */
    ...
    if ( flags&END_EDIT_REMOVEUI ) {      
        ...
        if (hMrBlueActions)
            DLSetWindowLongPtr( hMrBlueActions, 0);
        if (hMrBlueTriggers)
            DLSetWindowLongPtr( hMrBlueTriggers, 0);
        if (hMrBlueParamsHL)
            DLSetWindowLongPtr( hMrBlueParamsHL, 0);
        if (hMrBlueParamsVPT)
            DLSetWindowLongPtr( hMrBlueParamsVPT, 0);
        if (hMrBlueParamsAnim)
            DLSetWindowLongPtr( hMrBlueParamsAnim, 0);
    }
...
void MrBlueCreationManager::End()
{
    if ( mrBlueObject ) {
        ...
        if ( !attachedToNode ) {
			mrBlueObject->DeleteAllRefsFromMe();
			mrBlueObject->DeleteAllRefsToMe();
			mrBlueObject->DeleteThis();
            mrBlueObject = NULL;
        } else if ( mrBlueNode ) {
...
{  
    ...
    case MOUSE_POINT:
        ...
        case 0:
		{
            ...
            // link it up
            INode *l_mrBlueNode = createInterface->CreateObjectNode( mrBlueObject);
            attachedToNode = TRUE;
            assert( l_mrBlueNode );               
            createCB = mrBlueObject->GetCreateMouseCallBack();             
            createInterface->SelectNode( l_mrBlueNode );
            ...
            assert(targNode);
            targName = l_mrBlueNode->GetName();
			targName += GetString(IDS_DB_TARGET);
			macroRec->Disable();
			targNode->SetName(targName);
			macroRec->Enable();

            // hook up camera to target using lookat controller.
            createInterface->BindToTarget(l_mrBlueNode,targNode);

            // Reference the new node so we'll get notifications.
            ReplaceReference( 0, l_mrBlueNode);
