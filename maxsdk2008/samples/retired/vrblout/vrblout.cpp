...
MtlHasTexture(Mtl* mtl)
{
    ...
    if (bitmapFile.data() == NULL)
        return FALSE;
    int l = static_cast<int>(strlen(bitmapFile)-1);   // SR DCAST64: Downcast to 2G limit.
    if (l < 0)
        return FALSE;
...
WriteAppData(Interface* ip, int id, TCHAR* val)
{
    ...
    node->RemoveAppDataChunk(Class_ID(VRBL_EXPORT_CLASS_ID,0),
                             SCENE_EXPORT_CLASS_ID, id);
    TCHAR* buf = (TCHAR*) MAX_malloc(_tcslen(val)+1);
    _tcscpy(buf, val);
    node->AddAppDataChunk(Class_ID(VRBL_EXPORT_CLASS_ID,0),
                          SCENE_EXPORT_CLASS_ID, id,
                          static_cast<DWORD>(_tcslen(val)+1), buf);  // SR DCAST64: Downcast to 4G limit.
...
static DWORD HashNode(DWORD_PTR o, int size)
{
    return (o >> 2) % size;
}
...
{
    DWORD hash = HashNode((DWORD_PTR) node, NODE_HASH_TABLE_SIZE);
    NodeList* nList;
