...
int Reactor::NumRefs() 
{
	if (master != NULL && master->client == NULL)
		return 3; // one extra for remap on load.
	return 2;
}
...
void Reactor::SetReference(int i, RefTargetHandle rtarg) 
{
	switch (i)
	{
		...
		case 1:
			...
			break;
		case 2: // remapped on load.  (This is only applicable to loading pre-R7 files)
			if (master != NULL)
			{
				master->ReplaceReference(0, rtarg);
				if (rtarg != NULL)
				{
					// remove the reference after it has been set.  Note remove it directly instead of 
					// calling DeleteReference() because I don’t want GetReference(2) to return anything but NULL.
					RefList& refs = rtarg->GetRefList();
					if ( refs.first )
						refs.DeleteItem(this, rtarg->TestAFlag(A_NOTIFYDEP));
				}
			}
			break;
		default:
...
{ 
	...
	{
		case REFMSG_TARGET_DELETED:
			if (client && client->TestAFlag(A_BEING_AUTO_DELETED))
			{
				client = NULL;
...
{
	RefList& refs = this->GetRefList();
	RefListItem* refItem = refs.first;
	while (refItem)
	{
		Reactor* r = (Reactor*)GetIReactorInterface(refItem->maker);
		if (r != NULL)
		{
			...
			for(int x = r->slaveStates.Count()-1; x >= 0; x--)
			{
				// this is a seperate pass since DeleteReaction stores the entire undo record. 
