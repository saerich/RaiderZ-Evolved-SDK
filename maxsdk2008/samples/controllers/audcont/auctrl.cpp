...
float AudioBaseControl::SampleAtTime(TimeValue t, int ignore_oversampling, int painting)
{
	...
	if (wave->IsEmpty()) {
		...
		if (szFilename.isNull()) {
			...
			BOOL loaded = wave->InitOpen(szFilename);
			if(!loaded) {
				return minval;
			}
		}
...
public:
	...
	virtual int GetAssetType() const	{ return IAssetAccessor::kAnimationAsset; }
...
MaxSDK::Util::Path AudioBaseAccessor::GetPath() const	{
	return MaxSDK::Util::Path(mAudioController->szFilename);
}

void AudioBaseAccessor::SetPath(const MaxSDK::Util::Path& aNewPath)	{
	mAudioController->szFilename = aNewPath.GetString();
}
...
void AudioBaseControl::EnumAuxFiles(NameEnumCallback& nameEnum, DWORD flags)
{
	...
	if(flags & FILE_ENUM_ACCESSOR_INTERFACE)	{
		...
		MaxSDK::Util::Path path(szFilename);
